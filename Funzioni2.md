...
gsummary.num <- function(x, data=NULL, stats=F, 
                         ...
                         smoothCurve=T,  # set T to overlay smooth curve over rel
                         horizontalBoxplot=F  # # set T to use horizontal boxplot
                         ) {
  ...
  if("rel" %in% plots) {
    ...
    abline(v=mean(x), col="red")
    abline(v=median(x), col="green")
    if(isTRUE(smoothCurve)) {
      lines(density(x), col="blue")
    }
    
    if(isTRUE(normCurve)) {
      xnorm <- seq(min(x), max(x), length.out=2500)
      lines(xnorm, dnorm(xnorm, mean(x), sd(x)), col="red")
    }
  }
  
  if("box" %in% plots) {
    # boxplot
    boxplot(x, main=varname, horizontal = horizontalBoxplot)
  }
